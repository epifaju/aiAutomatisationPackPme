...
| IA externe (opt.) | OpenAI-compatible (`AI_PROVIDER=openai`) ou Anthropic (`anthropic`) | — |
...
Pour une API cloud : `AI_PROVIDER=openai` (+ `AI_OPENAI_*`) ou `AI_PROVIDER=anthropic` (+ `AI_ANTHROPIC_*`) — voir [docs/troubleshooting.md](docs/troubleshooting.md).
